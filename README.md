# Task-Tracker
A CLI app to track your tasks and manage your to-do list.
## Features:
You can add new tasks with a description, update existing task descriptions, delete tasks by ID, mark tasks as ‘todo’, ‘in-progress’, or ‘done’, list all tasks, or filter by specific status.
## How to Run:
To use the application, run it using the python command followed by the script name (task_tracker.py), your desired command, and any necessary arguments. Make sure to use the 'help' command in order to see all available commands.
